    ...
    switch (cmd) {
      ...
      case 'squash': {
        ...
        try {
          ...
          if (!branch || branch === 'main' || branch === 'master') {
            const msg = await el.db.addMessage(sess.id, 'system', 'Cannot squash on main/master. Switch to a feature branch first.')
            if (msg) store.appendMessage(parseMessage(msg))
            break
          }
          ...
          if (countStr) {
            ...
            if (isNaN(count) || count < 1) {
              const msg = await el.db.addMessage(sess.id, 'system', 'Usage: `/squash [number-of-commits]`')
              if (msg) store.appendMessage(parseMessage(msg))
              break
            }
          }
          const squashMsg = countStr ? `Squash ${count} commits on ${branch}` : `Squash all commits on ${branch}`
          const result = await el.git.squash(wsPath, count || 999, squashMsg)
          const msg = await el.db.addMessage(sess.id, 'system', `**${result}**`)
          if (msg) store.appendMessage(parseMessage(msg))
        } catch (err) {
          const msg = await el.db.addMessage(sess.id, 'system', `**Squash failed:** ${(err as Error).message}`)
          if (msg) store.appendMessage(parseMessage(msg))
        }
        break
      }
      ...
      case 'open-pr': {
        ...
        try {
          ...
          const result = await el.exec.run(workspacePath, 'gh pr view --web 2>&1')
          if (result.ok) {
            const msg = await el.db.addMessage(sess.id, 'system', '**Opened PR in browser.**')
            if (msg) store.appendMessage(parseMessage(msg))
          } else {
            // Check if it's "no PR" or "not authenticated"
            if (result.output.includes('no pull requests')) {
              const msg = await el.db.addMessage(sess.id, 'system', `**No PR found** for the current branch (\`${status.branch || 'detached'}\`). Push and create a PR first.`)
              if (msg) store.appendMessage(parseMessage(msg))
            } else if (result.output.includes('not logged in')) {
              const msg = await el.db.addMessage(sess.id, 'system', 'Not authenticated with GitHub CLI. Run `gh auth login` to set up.')
              if (msg) store.appendMessage(parseMessage(msg))
            } else {
              const output = result.output.length > 2000 ? result.output.slice(0, 2000) + '\n\n[truncated]' : result.output
              const msg = await el.db.addMessage(sess.id, 'system', `**Failed to open PR:**\n\n${output}`)
              if (msg) store.appendMessage(parseMessage(msg))
            }
          }
        } catch (err) {
      ...
      case 'focus': {
        const { workspacePath, fileNodes } = useAppStore.getState()
        if (!workspacePath) {
          const msg = await el.db.addMessage(sess.id, 'system', 'No workspace open. Use /folder to open a project first.')
          if (msg) store.appendMessage(parseMessage(msg))
          break
        ...
        const filePath = args.trim()
        if (!filePath) {
          const msg = await el.db.addMessage(sess.id, 'system', 'Usage: `/focus <file-path>` — navigate to and open a file in the editor')
          if (msg) store.appendMessage(parseMessage(msg))
          break
        }
        // Resolve relative path against workspace
        ...
        if (node) {
          store.openFileTab(node)
          const msg = await el.db.addMessage(sess.id, 'system', `**Opened:** \`${filePath}\``)
          if (msg) store.appendMessage(parseMessage(msg))
        } else {
          // Try partial match
          const matches = fileNodes.length > 0
            ...
            : []
          if (matches.length === 1) {
            store.openFileTab(matches[0])
            const msg = await el.db.addMessage(sess.id, 'system', `**Opened:** \`${matches[0].path.replace(workspacePath + '/', '')}\``)
            if (msg) store.appendMessage(parseMessage(msg))
          } else if (matches.length > 1) {
            const list = matches.slice(0, 10).map(m => `  \`${m.path.replace(workspacePath + '/', '')}\``).join('\n')
            const overflow = matches.length > 10 ? `\n  ... and ${matches.length - 10} more` : ''
            const msg = await el.db.addMessage(sess.id, 'system', `**${matches.length} files match** \`${filePath}\`:\n${list}${overflow}\n\nBe more specific.`)
            if (msg) store.appendMessage(parseMessage(msg))
          } else {
            const msg = await el.db.addMessage(sess.id, 'system', `**File not found:** \`${filePath}\``)
            if (msg) store.appendMessage(parseMessage(msg))
          }
        }
        break
...
function usePaletteCommands(
  ...
  const slashCommands: CommandEntry[] = [
    ...
    { id: 'cmd-open-pr', label: '/open-pr', description: 'Open current PR in browser', category: 'Slash Commands', action: () => handleCommand('open-pr', '') },
    { id: 'cmd-focus', label: '/focus', description: 'Navigate to file in editor', category: 'Slash Commands', action: () => handleCommand('focus', '') },
...
function collectFiles(nodes: import('@/types').FileNode[]): import('@/types').FileNode[] {
  const result: import('@/types').FileNode[] = []
  for (const n of nodes) {
