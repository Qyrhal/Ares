...
describe('InputBar — arrow key navigation in command picker', () => {
  ...
  it('ArrowDown from last item clamps at last (no wrap)', () => {
    ...
    const textarea = screen.getByPlaceholderText(PLACEHOLDER)
    fireEvent.change(textarea, { target: { value: '/' } })
    // 65 builtins, indices 0-64. After 47 presses from 0, highlight is at index 47 (/tree)
    for (let i = 0; i < 49; i++) {
      fireEvent.keyDown(textarea, { key: 'ArrowDown' })
    }
    // After 49 presses from 0, highlight is at index 49 (/tree)
    ...
    fireEvent.keyDown(textarea, { key: 'ArrowDown' })
    // Continue navigating to /help (index 66)
    for (let i = 0; i < 14; i++) {
      fireEvent.keyDown(textarea, { key: 'ArrowDown' })
    }
