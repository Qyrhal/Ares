...
function TerminalInstance({ id }: { id: string }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsub = window.electron.terminal.onOutput((tid, data) => {
  ...
  if (err) {
    return <div className="flex flex-1 items-center justify-center text-xs text-destructive">{err}</div>
  }
